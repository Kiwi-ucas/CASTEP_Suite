...
Seventeen Fortran source files in `src/` (17 compiled, 1 uncompiled development artifact), plus a Rust subproject:
...
14.1 **pes.f90** — `pes` module. Unified PES (Potential Energy Surface) scan — replaces `pes_scan.f90` (2D) and `pes3d.f90` (3D). Defines `pes_grid_t` type with `ndim` field (2 or 3), shared fields for scan plane, grid dimensions, fractional range, mobile atom, scan mode, and symmetry. Public: `generate_pes_grid_points` — dispatches to 2D or 3D grid generation based on `ndim`, `compute_local_grid_bounds` — symmetry half-distance calculation (3D only), `write_pes_cube` — unified Gaussian Cube output (2D with `nz=1`; line-2 carries full-key JSON metadata with plane, mobile atom, lattice, ranges), `collect_pes_energies` — parses cube header for dimensions, scans grid subdirectories, rewrites cube with energies (NaN for missing), `symmetry_expand_energies` — forward symmetry ops to expand local 3D grid to full cell. Private: `find_castep_in_dir`, `parse_castep_energy`, `element_to_z`, `compute_lattice_vectors`, `extract_json_int`, `wrap_to_unit`. Depends on `castep_config` only. Old `pes_scan.f90` and `pes3d.f90` are thin wrappers for backward compatibility.
